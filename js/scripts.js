...
var pokemonRepository = (function() {
  ...
  function addListItem(pokemon) {
    ...
    var $button = $('<button type="button" class="btn btn-primary btn-outline-danger" data-toggle="modal" data-target="#exampleModalCenter"</button>');
  ...
  function showModal(item) {
    ...
    var pokemonHeight = $('<p class="pokemon-height"></p>').text(item.name + ' is ' + item.height/10 + ' meters tall. ');
    //create a paragraph for the pokemon weight
    var pokemonWeight = $('<p class="pokemon-Weight"></p>').text(item.name + ' weighs ' + item.weight/100 + ' grams. ');
    //render pokemon Image to appear in the modal
    var pokemonImage = $('<img class="modal-body" src="' + item.imageUrl + '">');
